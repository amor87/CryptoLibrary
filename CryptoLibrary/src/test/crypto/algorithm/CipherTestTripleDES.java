...
public class CipherTestTripleDES {
	...
	@org.junit.Before
	public void setUp() {
		encryptor = new Encryptor(AlgorithmE.TRIPLE_DES, BlockCipherModeE.ECB);
		decryptor = new Decryptor(AlgorithmE.TRIPLE_DES, BlockCipherModeE.ECB);
	}
	...
	private aj.crypto.model.Key createKey() {
		String formatted3DESKey = "7eSt-Key7eSt-Key7eSt-Key";
		org.junit.Assert.assertEquals(24, formatted3DESKey.length());
		return Key.GenerateByGivenFormattedKey(formatted3DESKey);
	}

	// External Online Tool https://www.tools4noobs.com/online_tools/encrypt/
	@org.junit.Test
	public void testDecryptHexDataFromExternalTool() {
		String key = "x7rNLk3yx7rNLk3yx7rNLk3y";
	...
	public void testDecryptBase64DataFromExternalTool() {
		...
		org.junit.Assert.assertEquals("another text encrypted from tools4noob.com web page with base64 output",
				decryptedText);
	}
